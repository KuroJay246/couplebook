...
function main() {
  ...
  log('firestore.rules is the authoritative production Firestore rules file.');

  if (firestoreRules.includes('<JAYLAN_UID>') || firestoreRules.includes('<PARTNER_UID>') || storageDraft.includes('<JAYLAN_UID>') || storageDraft.includes('<PARTNER_UID>')) {
    hasFailure = true;
