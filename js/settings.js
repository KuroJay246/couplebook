...
function initSettingsMenu() {
  ...
  const verifyUnlock = () => {
    ...
    const SETTINGS_PIN = 'kurojay'; // Dedicated settings unlock PIN

    if (password === SETTINGS_PIN) {
      isUnlocked = true;
      ...
      if (errorMsg) errorMsg.textContent = 'Incorrect PIN. Access denied.';
      passInput.value = '';
    }
  };

  document.getElementById('btn-settings-auth-submit')?.addEventListener('click', verifyUnlock);
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      const confirmReset = confirm(
        "Are you sure you want to clear your MemoryBook? This will wipe all custom memories, profiles, settings, and contract acceptance states."
      );
      if (confirmReset) {
        localStorage.clear();
        alert("Storage cleared successfully. Application will now restart.");
