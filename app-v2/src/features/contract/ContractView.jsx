...
function AgreementSection({ agreement, agreementReady, agreementSections }) {
  return (
    <Surface>
      <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--cb-accent)]">Agreement</p>
      <h3 className="mt-2 font-serif text-3xl text-[var(--cb-text)]">{agreement?.title || 'Our agreement'}</h3>
      <p className="mt-3 text-sm leading-6 text-[var(--cb-text-secondary)]">{agreement?.introduction}</p>
      <div className="mt-5 grid gap-4">
        {agreementReady ? agreementSections.map((section) => (
...
function AcceptanceSection({ acceptanceCards }) {
  return (
    <Surface tone="soft">
...
function PrivacyBoundarySection({ privacyItems }) {
  return (
    <Surface tone="soft">
...
function ContractHistorySection({ history }) {
...
function getContractRouteState({ compatibilityError, compatibilityState, model, onRefresh }) {
...
function ContractActions({ accepted, agreementReady, onAccept, saving, version }) {
  return (
    <>
      {version ? <StatusBadge tone="info">Version {version}</StatusBadge> : null}
      <PrimaryButton disabled={accepted || !agreementReady} loading={saving} onClick={onAccept}>
...
function ContractAlerts({ sourceWarnings = [], status }) {
  return (
    <>
      ...
      {sourceWarnings.length ? (
        <InlineAlert
          ...
          description={`The current Contract view loaded with ${sourceWarnings.length} review note${sourceWarnings.length === 1 ? '' : 's'}.`}
        />
      ) : null}
    </>
  )
...
export function ContractView({ compatibilityError, compatibilityState, model, onRefresh }) {
  ...
  const privacyItems = model.privacy?.items || []
  ...
  const routeState = getContractRouteState({ compatibilityError, compatibilityState, model, onRefresh })
      ...
      <PageHeader
        ...
        actions={(
          <ContractActions
            ...
            version={model.agreement?.version}
          />
        )}
      />

      <ContractAlerts sourceWarnings={model.sourceStatus?.warnings || []} status={status} />

      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]">
        <AgreementSection agreement={model.agreement} agreementReady={agreementReady} agreementSections={agreementSections} />

        <div className="grid gap-5">
          <AcceptanceSection acceptanceCards={acceptanceCards} />
          <PrivacyBoundarySection privacyItems={privacyItems} />
        </div>
      </div>

      <ContractHistorySection history={history} />
