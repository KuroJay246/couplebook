...
test('settings read model keeps approved identity, appearance notes, and migration progress explicit', () => {
  ...
  assert.equal(model.media.title, 'Google Drive media library')
  ...
  assert.equal(model.media.items[3].meta, 'Setup needed')
  ...
  assert.match(model.media.backendReadiness.description, /media service pieces are prepared locally/i)
...
test('settings read model normalizes notification preferences and permission copy without prompting', () => {
  ...
  assert.match(model.notifications.backendBoundary, /approved private notification service/)
})
