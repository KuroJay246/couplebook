...
test('route source and auth shell source keep the protected migration contract explicit', async () => {
  ...
  assert.equal(routesSource.indexOf('path="/update"') < routesSource.indexOf('<ProtectedRoute />'), true)
  assert.equal(routesSource.indexOf('path="/maintenance"') < routesSource.indexOf('<ProtectedRoute />'), true)
  assert.match(routesSource, /<MaintenanceGate \/>/)
  ...
  assert.match(appProvidersSource, /AuthProvider/)
