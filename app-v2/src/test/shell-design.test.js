...
test('shared shell styles use the Event Hub-family token set adapted to Couple Book', async () => {
  ...
  assert.match(styleSource, /--cb-color-bg:/)
  assert.match(styleSource, /--cb-color-primary:/)
  assert.match(styleSource, /mobile-tab-bar/)
  ...
  assert.doesNotMatch(styleSource, /--paper:/)
})
