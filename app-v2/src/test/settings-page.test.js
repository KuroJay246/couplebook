...
test('settings route uses the read-only feature hook and utility view', async () => {
  ...
  assert.match(settingsViewSource, /Relationship & Profiles/)
  assert.match(settingsViewSource, /Important Dates/)
  assert.match(settingsViewSource, /Media & Sync/)
  assert.match(settingsViewSource, /Notifications/)
  assert.match(settingsViewSource, /App & Storage/)
  assert.match(settingsViewSource, /Advanced \/ Diagnostics/)
  assert.match(settingsViewSource, /About/)
  assert.match(settingsViewSource, /Permission is not requested automatically/)
  assert.doesNotMatch(settingsViewSource, /requestPermission\(/)
  assert.match(mediaSettingsSource, /Open Album/)
  assert.match(mediaSettingsSource, /Open Shared Album/)
  assert.match(mediaSettingsSource, /rel="noreferrer noopener"/)
  assert.match(mediaSettingsSource, /target="_blank"/)
  ...
  assert.doesNotMatch(settingsViewSource, /Birthday, Valentine, and Confession|Open Contract/)
...
test('settings view keeps raw technical details and old static dependencies out of the migrated route', async () => {
  ...
  assert.match(settingsSelectorsSource, /normalizeSharedIcloudAlbumUrl/)
  assert.match(settingsSelectorsSource, /VITE_SHARED_ICLOUD_ALBUM_URL/)
...
test('settings account controls preserve existing Firebase identity while linking Google', async () => {
  ...
  assert.match(settingsViewSource, /approved identity, couple membership, and private book data stay intact/)
  assert.doesNotMatch(settingsViewSource, /current Firebase UID/)
})
