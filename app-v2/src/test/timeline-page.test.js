...
test('timeline route uses the read-only feature hook and story view', async () => {
  ...
  assert.match(timelineHookSource, /memorySource: memory\.source/)
  assert.match(timelineHookSource, /mediaIndexSource: mediaIndex\.source/)
  ...
  assert.match(storyPreviewHookSource, /URL\.createObjectURL/)
