...
| Drive media index | Firestore `couples/{coupleId}/mediaItems/{mediaId}` and `couples/{coupleId}/mediaSync/google-drive` | Stable Drive IDs, MIME/media type, dimensions, duration, size, checksum, safe captions, timestamps, sync cursor/health | Confirm no `thumbnailLink`, `webContentLink`, temporary preview URL, access token, refresh token, signed URL, or raw local path is present | Rebuild from Drive file listing or restore stable records into emulator first, then apply reviewed document-scoped metadata migration |
...
- Continuous Drive sync requires an owner-approved trusted backend for OAuth code exchange, refresh credential storage, Drive Changes processing, webhook handling, watch renewal, scheduled reconciliation, thumbnail mediation, and privacy-minimal audit writes.
- Do not include Google OAuth refresh credentials in Firestore backups, media manifests, frontend source, screenshots, or owner-review reports.
